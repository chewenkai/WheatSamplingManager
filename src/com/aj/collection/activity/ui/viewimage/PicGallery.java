package com.aj.collection.activity.ui.viewimage;
...
public class PicGallery extends Gallery {
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
		...
		if (view instanceof MyImageView) {
			...
			float min_distance = PictureViewActivity.screenWidth / 4f;
			...
			if ((int) width <= PictureViewActivity.screenWidth
					&& (int) height <= PictureViewActivity.screenHeight)// 如果图片当前大小<屏幕大小，直接处理滑屏事件
			{
				...
				if (distanceX > 0)// 向左滑动
				{
					if (r.left > 0 || right < PictureViewActivity.screenWidth) {// 判断当前ImageView是否显示完全
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
				...
				{
					if (r.right < PictureViewActivity.screenWidth || left > 0) {
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
						imageView.postTranslate(-distanceX, -distanceY);
					}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (view instanceof MyImageView) {
				...
				if ((int) width <= PictureViewActivity.screenWidth
						&& (int) height <= PictureViewActivity.screenHeight)// 如果图片当前大小<屏幕大小，判断边界
				{
					break;
				}
				float v[] = new float[9];
				...
				if (top < 0 && bottom < PictureViewActivity.screenHeight) {
...
//							- bottom, 200f);
				...
				if(left<0 && right< PictureViewActivity.screenWidth){
