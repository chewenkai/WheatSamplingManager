...
class TypePhotos(var mContext: Context, var sheetCell: SheetCell, val autoGeneratedSheetID:String): CellBaseAttributes() {
    ...
    override fun get_cell_value(): String {
        return MediaManager().getLatestCameraFiles(mContext, autoGeneratedSheetID).toString()
    }
    ...
    override fun getJsonContent(): JSONObject {
        // 转换图片名字为Json
        val photoFiles = MediaManager().getLatestCameraFiles(mContext, autoGeneratedSheetID)
        val jsonArray = JSONArray()
        for (index in photoFiles.indices) {
            jsonArray.put(index, photoFiles[index].name)
        }
        // 将内容生成Json
        val json = JSONObject()
    ...
    init {
        ...
        gallaryAdaper = GallaryAdaper(mContext,MediaManager().getLatestCameraFiles(mContext, autoGeneratedSheetID), autoGeneratedSheetID)
        ...
        takePhotoButton!!.onClick {
            ...
            if (location_info_type == BDLocation.TypeGpsLocation ||
                ...
                intent.putExtra("root_path", Util.getMediaFolder(mContext)+File.separator+autoGeneratedSheetID)    //将存储图片根目录传递过去
    ...
    class GallaryAdaper(var mContext: Context, private val photoFiles: ArrayList<File>, val autoGeneratedSheetID:String) : RecyclerView.Adapter<GallaryAdaper.ViewHolder>() {
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            image.setOnClickListener {
                ...
                intent.putExtra(Constant.AUTO_GENERATED_SHEETID, autoGeneratedSheetID)
        ...
        fun updateUI() {
            ...
            this.photoFiles.addAll(MediaManager().getLatestCameraFiles(mContext, autoGeneratedSheetID))
