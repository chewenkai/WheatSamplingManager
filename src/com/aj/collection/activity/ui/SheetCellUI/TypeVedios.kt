...
import com.aj.collection.activity.tools.Util.getBitmapUri
...
class TypeVedios(var mContext: Context, var sheetCell: SheetCell, val autoGeneratedSheetID:String): CellBaseAttributes() {
    ...
    override fun get_cell_value(): String {
        return MediaManager().getLatestVedioFiles(mContext, autoGeneratedSheetID).toString()
    }
    ...
    override fun getJsonContent(): JSONObject {
        // 转换视频名字为Json
        val vedioFiles = MediaManager().getLatestVedioFiles(mContext, autoGeneratedSheetID)
        val jsonArray = JSONArray()
        for (index in vedioFiles.indices) {
            jsonArray.put(index, vedioFiles[index].name)
        }
        // 将内容生成Json
        val json = JSONObject()
        json.put(SheetProtocol().CELL_NAME, get_cell_name())
        json.put(SheetProtocol().CELL_TYPE, get_cell_type())
        json.put(SheetProtocol().CELL_VALUE, jsonArray)
    ...
    override fun isFilled(): Boolean {
        if (sheetCell.cell_fill_required==(SheetProtocol().False))
            ...
            val photosNumber = MediaManager().getLatestCameraFiles(mContext, autoGeneratedSheetID).size
    ...
    init {
        ...
        gallaryAdaper = GallaryAdaper(mContext,MediaManager().getLatestVedioFiles(mContext, autoGeneratedSheetID), autoGeneratedSheetID)
        ...
        takeVedioButton!!.onClick {
            ...
            if (location_info_type == BDLocation.TypeGpsLocation ||
                ...
                val video_name = File(Util.getMediaFolder(mContext)+File.separator+autoGeneratedSheetID + File.separator + "VIDEO_" + (mContext.getSystemService(Context.TELEPHONY_SERVICE) as TelephonyManager).deviceId + Util.getCurrentTime("yyMMddHHmmss") + ".mp4")
    ...
    class GallaryAdaper(var mContext: Context, private val photoFiles: ArrayList<File>, val autoGeneratedSheetID:String) : RecyclerView.Adapter<GallaryAdaper.ViewHolder>() {
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            if (ContextCompat.checkSelfPermission(mContext, Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
                Picasso.with(mContext).load(getBitmapUri(mContext, createVideoThumbnail(photoFile.path, 0))).resize(0,
        ...
        fun updateUI() {
            ...
            this.photoFiles.addAll(MediaManager().getLatestVedioFiles(mContext, autoGeneratedSheetID))
