			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				...
				if (!s.toString().equals("")) {
					...
					for (int i = 0; i < mResultsValues.size(); i++) {
						if (s1.toString().equals(mResultsValues.get(i))) {
							mResultsValues.remove(i);
							mResultsValues.add(s.toString());
							return;
						}
					}

					if(!mResultsValues.contains(s.toString()))
						mResultsValues.add(s.toString());
				}
			}
